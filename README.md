...
#### 5. サービスアカウントキーの作成と権限設定

**重要**: サービスアカウントにVision APIを使う権限を付与する必要があります。

**ステップ1: サービスアカウントの作成**
1. Google Cloud Console で左メニュー「IAMと管理」→「サービスアカウント」
2. 「サービスアカウントを作成」をクリック
3. 名前を入力（例：`paper-manager-sa`）
4. 説明を入力（例：`Paper Manager用のサービスアカウント`）
5. 「作成して続行」をクリック

**ステップ2: ロール（権限）の付与**
1. 「このサービスアカウントにプロジェクトへのアクセスを許可する」画面で、以下のロールを追加：
   - **必須**: `Cloud Vision API ユーザー` または `Cloud Vision API 管理者`
     - 検索ボックスに「Vision」と入力して選択
   - **推奨**: `ストレージ オブジェクト閲覧者`（PDFアップロードがある場合）
2. 「続行」をクリック
3. 「完了」をクリック
...
5. ダウンロードされたJSONファイルをPaper Managerフォルダに保存（例：`service-account-key.json`）
6. GUI設定画面の「Google Cloud認証ファイル」にファイルパスを入力

**トラブルシューティング**:
- **「Permission denied」エラーが出る場合**: ロール設定を再確認してください
- **JSONファイルが見つからない**: ダウンロードフォルダを確認し、適切な場所に移動してください
